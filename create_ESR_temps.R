...
library(tidyverse)
total_mean_SST <- mean(temperature_summary$surface_temp[which(temperature_summary$year >= 1994 & temperature_summary$year <= 2014)], na.rm=TRUE)
total_mean_BT <- mean(temperature_summary$temp200m[which(temperature_summary$year >= 1994 & temperature_summary$year <= 2014)], na.rm=TRUE)
