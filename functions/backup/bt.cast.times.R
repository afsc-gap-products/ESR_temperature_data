bt.cast.times <- function(survey = "GOA", bt.casts = NA, user.name = 'lamane', AFSC.pwd = 'maculata_480'){
...
# March 2015
#
# The cast.times function gathers data from Oracle and Stan's offset tables and ultimately
# generates a table of event times defining a trawl event.  These events are downcast start,
# downcast end, on bottom, haul back, off bottom, upcast start and upcast end; they bracket
# the portion of the trawl haul event that is of interest to us for the use of MK9 irradiance
# data.  Within this function MK9 light meter depths and times are adjusted to using the info
# provided in the offsets table so that MK9 depth is calibrated to the survey MBT and MK9
# clock drift is adjusted to synch up with the MBT/GPS time that is the basis for the survey
# clock.
#
#Dependencies:
#
...
#
#Functions:
# get.sgt.data - from Oracle
# get.mbt.data - from Oracle
# get.haul.list - from Oracle
# get.regr - from offsets table
#
#Key to MK9 performance codes:
# 0 = good performane
# 1 = bad performance
# -99 = no data between on and off bottom events
#
# Objects created:
# MK9.performance.csv - trawl-mounted light meter deployment performance code
# CastTimes.csv - catalog of events during trawl haul operation
#
############################################################################################
...
# survey="BS"
# user.name='lamane'
# AFSC.pwd='entomelas_30220'

## select Survey, Vessel, and Cruise to populate Oracle queries
# survey <- readline("Enter Survey Designation (e.g., AI, BS, GOA, NBS, or Slope):  ");
# survey <- toupper(survey)
# vessel <- readline("Enter vessel code (e.g., 162 for Alaska Knight):  ");
# vessel <- as.numeric(vessel)
# cruise <- readline("Enter Cruise ID (e.g., 200501):  ");
# cruise <- as.numeric(cruise)

## open connection to Oracle
channel <- odbcConnect(dsn = "AFSC", uid = user.name, pwd = AFSC.pwd, believeNRows = FALSE)

## cruise year vector
years = sort(unique(floor(bt.casts$cruise/100)))
...
# mbt = get.mbt.data(survey,vessel,cruise,channel)
# replaced by get.bt.casts() which collects all casts for a region
sgt = get.sgt.data(survey,vessel,cruise,channel)

hauls = get.haul.list(survey,vessel,cruise,channel)
## getting values from regression to correct light meter times
coef <- get.regr(survey,vessel,cruise,channel)
...
light.loc <- paste("G:\\RACE_LIGHT\\LightData\\Data\\year_", yy, "\\", sur, "\\v_", vessel, sep = "")
...
file.idx2 <- which(substr(files.in.dir, 1, 4) == "offs")
...
haullist <- merge(offsets, start_time); haullist <- merge(haullist, end_time)
